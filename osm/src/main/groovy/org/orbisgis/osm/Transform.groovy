    ...
    return create({
        ...
        run { datasource, osmTablesPrefix, epsgCode, tag_keys ->
            ...
            if (datasource != null) {
                ...
                def pointsNodes = extractNodesAsPoints(datasource, osmTablesPrefix, epsgCode, outputTableName, tag_keys)
                if (pointsNodes) {
                    info "The points have been built."
                } else {
                    info "Cannot extract any points."
                    return
                }
            } else {
    ...
    return create({
        ...
        run { datasource, osmTablesPrefix, epsgCode, tag_keys ->
            ...
            if (datasource != null) {
                ...
                info "Indexing osm tables..."
                buildIndexes(datasource, osmTablesPrefix)
                def lineWays = extractWaysAsLines(datasource, osmTablesPrefix, epsgCode, outputTableName, tag_keys)
                if (lineWays) {
                    info "The lines have been built."
                } else {
    ...
    return create({
        ...
        run { datasource, osmTablesPrefix, epsgCode, tag_keys ->
            ...
            if (datasource != null) {
                ...
                info "Indexing osm tables..."
                buildIndexes(datasource, osmTablesPrefix)
                def ouputWayPolygons = "WAYS_POLYGONS_$uuid"
                def polygonWays = extractWaysAsPolygons(datasource, osmTablesPrefix, epsgCode, ouputWayPolygons, tag_keys)
                def ouputRelationPolygons = "RELATION_POLYGONS_$uuid"
                def polygonRelations = extractRelationsAsPolygons(datasource, osmTablesPrefix, epsgCode, ouputRelationPolygons, tag_keys)
                if (polygonWays && polygonRelations) {
                    ...
                    info "The polygons have been built."
                } else if (polygonWays) {
                    ...
                    info "Cannot extract any polygons."
                    return
                }
            } else {
                error "Please set a valid database connection"
...
static boolean extractWaysAsPolygons(JdbcDataSource dataSource, String osmTablesPrefix, int epsgCode,
                                     String ouputWayPolygons, def tag_keys) {
    ...
    if (rows.count > 0) {
        ...
        error "No keys or values founded in the ways"
        return false
    }
}
...
static boolean extractRelationsAsPolygons(JdbcDataSource dataSource, String osmTablesPrefix, int epsgCode,
                                          String ouputRelationPolygons, def tag_keys) {
    ...
    if (rows.size() > 0) {
        ...
        if (list.isEmpty()) {
            ...
            CREATE TABLE ${ouputRelationPolygons} AS SELECT 'r'||a.id_relation as id, a.the_geom from ${
                RELATIONS_MP_HOLES
            } as a, ${
                osmTablesPrefix
            }_relation_tag  b where a.id_relation=b.id_relation group by a.the_geom, a.id_relation;"""
        } else {
            dataSource.execute """drop table if exists ${ouputRelationPolygons};     
                CREATE TABLE ${ouputRelationPolygons} AS SELECT 'r'||a.id_relation as id, a.the_geom, ${
                list.join(",")
            } from ${RELATIONS_MP_HOLES} as a, ${
                osmTablesPrefix
            }_relation_tag  b where a.id_relation=b.id_relation group by a.the_geom, a.id_relation;"""
        }
        ...
        error "No keys or values founded in the relations"
        return false
    }
}
...
static boolean extractWaysAsLines(JdbcDataSource dataSource, String osmTablesPrefix, int epsgCode,
                                  String outputWaysLines, def tag_keys) {
    def countTagKeysQuery = "select count(*) as count from ${osmTablesPrefix}_way_tag"
    def filterByKeys = false
    def whereKeysFilter
    if (tag_keys != null && !tag_keys.isEmpty()) {
    ...
    if (rows.count > 0) {
        ...
        if (list.isEmpty()) {
            ...
            dataSource.execute """drop table if exists ${outputWaysLines};
            CREATE TABLE ${outputWaysLines} AS SELECT 'w'||a.id_way as id, a.the_geom, ${list.join(",")} 
        ...
        if (list.isEmpty()) {
            if (filterByKeys) {
                dataSource.execute """drop table if exists ${ouputNodesPoints}; 
                    ...
                    ${epsgCode}) as the_geom, ${list.join(",")} from ${osmTablesPrefix}_node as a, 
                    ${osmTablesPrefix}_node_tag  b where a.id_node=b.id_node and b.${whereKeysFilter} group by a.id_node;"""
            } else {
                dataSource.execute """drop table if exists ${ouputNodesPoints}; 
                    CREATE TABLE ${ouputNodesPoints} AS SELECT a.id_node,ST_TRANSFORM(ST_SETSRID(a.THE_GEOM, 4326), 
                    ${epsgCode}) as the_geom, ${list.join(",")} from ${osmTablesPrefix}_node as a, 
